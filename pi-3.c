/**
  * Alumno: Ferrusca Ortiz Jorge Luis
  * Tarea 2
  * Calculo de PI (mpi).
...
int main(int argc, char** argv) {
  ...
  for (i = (offset * rank)+1; i <= ((offset * rank) + offset); ++i)
  {
    printf("Proceso %d computando n = %d \n", rank, i);
    x = h * ((double)i - 0.5);
    sum += f(x);
  }
  // Last process checking for remainder operations
  if (rank == size-1 && remainder != 0) {
    for (i = (offset * size) + 1; i <= n; ++i)
    {
      printf("Proceso %d computando remanente n = %d \n", rank, i);
      x = h * ((double)i - 0.5);
      sum += f(x);   
    }
  }
